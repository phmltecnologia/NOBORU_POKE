...
-- Cardápio: leitura pública dos itens ativos
CREATE POLICY menu_items_public_read ON menu_items
  FOR SELECT
  USING (active = true);

-- Perfis/sessões: somente service role (Edge Functions)
CREATE POLICY profiles_service_only ON profiles
  FOR ALL
  USING (false);
...
-- Storage bucket menu-images (criar via dashboard ou API; políticas abaixo se bucket existir)
...
-- Seed cardápio (DEFAULT de js/data.js)
...
-- Funções RPC para auth (bcrypt via pgcrypto)
